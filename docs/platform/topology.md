...
`GET /api/v1/discovery` returns observations from the last 24 hours, up to 100 identifiers per active gateway. Active registrations are excluded before the limit is applied. Withdrawn registrations may appear again. Unknown BLE advertisements are included even when their payload cannot be decoded. A reported model matching the catalog gets its product name and photo; a profile inferred only from signal kind is explicitly labeled as a suggestion requiring verification.

Verification: `TestDiscoveryRegistrationAndGatewayIsolation` covers discovery under multiple gateways, reported model/photo matching, hiding registered identifiers, rediscovery after withdrawal, tenant isolation, the time window, and unknown advertisements. Frontend build and lint completed (lint has image-element warnings). The updated local API was started on 2026-09-21 and passed readiness and authenticated discovery checks; at that check it returned no eligible discoveries. Physical-device registration through the browser remains unverified.
